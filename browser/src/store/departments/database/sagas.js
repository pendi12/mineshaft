...
function* databaseWrite({payload, metadata, config}) {
  /*--- Variables ---*/
  let request = {}
  /*--- Destructure ---*/
  const { entity, branch } = metadata
  /*--- Defaults ---*/
  const writeType = config.writeType && typeCorrect(config.writeType) ? config.writeType : DATABASE_DEFAULT_WRITE_TYPE
